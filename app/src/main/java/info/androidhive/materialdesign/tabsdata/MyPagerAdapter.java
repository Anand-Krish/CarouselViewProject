...
ViewPager.OnPageChangeListener {
	...
	private static float minAlpha=0.15f;//0.6f;
	private static float maxAlpha=1f;
	private static float minDegree=60.0f;
	private int counter=0;
	private FragmentTagListener fragmentTagListener;
	...
	public MyPagerAdapter(Context context, FragmentManager fm, FragmentTagListener fragmentTagListener, Myviewpager pager) {
		super(fm);
		this.fm = fm;
		this.context = context;
		this.fragmentTagListener = fragmentTagListener;
		this.pager = pager;
	}
	...
	public Fragment getItem(int position)
	{
		...
		if (position == OneFragment.FIRST_PAGE)
			scale = OneFragment.BIG_SCALE;
		else
		{
			scale = OneFragment.SMALL_SCALE;
			IsBlured=true;
		}
			...
			int positionOffsetPixels) 
	{
		...
		//		ViewHelper.setRotationY(nextnext, -minDegree);
			...
			if(cur!=null)
			{
				cur.setScaleBoth(OneFragment.BIG_SCALE
						- OneFragment.DIFF_SCALE * positionOffset);
		...
		//		ViewHelper.setRotationY(cur, 0);
			...
			if(next!=null)
			{
				next.setScaleBoth(OneFragment.SMALL_SCALE
						+ OneFragment.DIFF_SCALE * positionOffset);
 ...
 */
	...
	{
		//return ((SimpleTabsActivity)context).getFragmentTag(position);
		//return fragmentTagListener.onGetFragmentTag(position);
		return "android:switcher:" + pager.getId() + ":" + position;
	}

	public interface FragmentTagListener{
